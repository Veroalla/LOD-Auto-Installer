...
dim boxresult
boxresult = MsgBox ("Are you sure you want to uninstall LOD?  This will reset your addons folder to empty.", vbYesNo, "Uninstall LOD?")
...
xHttp.Open "GET", "https://www.dropbox.com/s/c85txj4h4qf3aec/lod.txt?dl=1", False
xHttp.Send
...
If (objFSO.FileExists(appDataLocation & "\lod.txt")) Then
  objFSO.DeleteFile(appDataLocation & "\lod.txt")
  Wscript.echo "Deleted old version lod.txt"
End If

If (objFSO.FileExists(appDataLocation & "\lod.zip")) Then
  objFSO.DeleteFile(appDataLocation & "\lod.zip")
  Wscript.echo "Deleted lod.zip"
End If
...
Wscript.echo "UNINSTALLING LOD"
For each path In steampaths
	if path <> "" then
		if objFSO.FolderExists(path & "\steamapps\common\dota 2 beta\dota\addons") then
			found = True
			Wscript.echo "Uninstalling from path: " & path & "\steamapps\common\dota 2 beta\dota\addons"
			
      objFSO.DeleteFolder(path & "\steamapps\common\dota 2 beta\dota\addons")
		end if 
	end if
Next
